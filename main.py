...
def create_augmented_dataloader(args, dataset):
    ...
    # 1) Original train (tokenized)
    orig_train = dataset["train"]
    orig_tokenized = orig_train.map(
        tokenize_function,
        batched=True,
    ...
    # 2) 5k random transformed examples from the training set
    transformed_subset = dataset["train"].shuffle(seed=42).select(range(5000))
    transformed_subset = transformed_subset.map(
        custom_transform,
        load_from_cache_file=False
    )
    transformed_tokenized = transformed_subset.map(
        tokenize_function,
        batched=True,
        load_from_cache_file=False
    )
    ...
    # 3) Concatenate original + transformed → augmented training set
    augmented_train = concatenate_datasets([orig_tokenized, transformed_tokenized])
